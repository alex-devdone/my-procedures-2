...
- [x] Update `apps/web/src/lib/local-todo-storage.ts` to store subtasks for each todo
- [x] Add unit tests for local subtask storage in `apps/web/src/lib/local-todo-storage.test.ts`
...
- [x] Add procedure to complete recurring todo (creates next occurrence, marks current complete)
...
- [x] Update `apps/web/src/app/api/todo/todo.hooks.ts` to handle scheduling in optimistic updates
- [x] Add unit tests for scheduling hooks in `apps/web/src/app/api/todo/todo.hooks.test.ts`
- [x] Update `apps/web/src/lib/local-todo-storage.ts` to store scheduling fields
- [x] Add unit tests for local storage scheduling in `apps/web/src/lib/local-todo-storage.test.ts`
...
- [x] Create `apps/web/src/hooks/use-reminder-checker.ts` - hook that polls for due reminders and triggers notifications
...
- [x] Create E2E tests for reminder notifications in localStorage mode at `apps/web/e2e/reminders.spec.ts`:
  - [x] E2E: Request browser notification permission
  - [x] E2E: Show in-app toast when reminder is due
  - [x] E2E: Dismiss reminder toast
  - [x] E2E: Click reminder toast to navigate to todo
...
- [x] Create `apps/web/src/components/views/upcoming-view.tsx` - shows todos due in next 7 days grouped by date
...
- [x] Create E2E tests for smart views in localStorage mode at `apps/web/e2e/smart-views.spec.ts`:
  - [x] E2E: Navigate to Today view and see only today's todos
  - [x] E2E: Navigate to Upcoming view and see todos grouped by date
  - [x] E2E: Navigate to Overdue view and see only past-due todos
  - [x] E2E: Complete todo from smart view and verify it updates
  - [x] E2E: Empty state shown when no todos match view criteria
...
- [x] Create `apps/web/src/hooks/use-folder-realtime.ts` - subscribes to folder table changes
- [x] Add unit tests for useFolderRealtime in `apps/web/src/hooks/use-folder-realtime.test.tsx`
- [x] Update `apps/web/src/app/api/folder/folder.hooks.ts` to integrate realtime subscription
...
- [x] Run `bun run check-types` and fix any TypeScript errors (0 errors)
- [x] Run `bun run check` and fix any linting issues (0 issues)
- [x] Run `bun run test` and ensure all unit tests pass (1864 tests passed, 17 skipped)
- [x] Run `bun run test:e2e` and ensure all E2E tests pass (138 tests passed)
